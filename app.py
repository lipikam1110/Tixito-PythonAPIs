...
def update_ticket_price(ticket_id):
    ...
    if not data or 'sellingPrice' not in data:
        return jsonify({"error": "Bad Request: 'sellingPrice' not provided", "status_code": 400}), 400
    ...
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute('UPDATE tickets SET sellingPrice = %s WHERE id = %s RETURNING *;', (selling_price, ticket_id))
        updated_ticket = cursor.fetchone()
        conn.commit()
        cursor.close()
        conn.close()
...
def create_ticket():
    ...
    try:
        cursor.execute("""
            INSERT INTO public."tickets" (actualPrice, comment, eventVenuesID, imageURL, listingPrice, lock, ordersID, sellerID, sellingPrice, status, type, validatedOn, verified)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING *;
        """, (actualPrice, comment, eventVenuesID, imageURL, listingPrice, lock, ordersID, sellerID, sellingPrice, status, type, validatedOn, verified))
...
def get_tickets():
    ...
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute('SELECT * FROM public."tickets";')
        tickets = cursor.fetchall()
        cursor.close()
        conn.close()
...
def get_ticket_by_id(ticket_id):
    ...
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute('SELECT * FROM public."tickets" WHERE id = %s;', (ticket_id,))
        ticket = cursor.fetchone()
        cursor.close()
        conn.close()
...
def delete_ticket_by_id(ticket_id):
    ...
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute('DELETE FROM public."tickets" WHERE id = %s RETURNING *;', (ticket_id,))
        deleted_ticket = cursor.fetchone()
        conn.commit()
        cursor.close()
        conn.close()
...
def update_ticket_price(ticket_id):
    ...
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute('UPDATE public."tickets" SET sellingPrice = %s WHERE id = %s RETURNING *;', (selling_price, ticket_id))
        updated_ticket = cursor.fetchone()
        conn.commit()
        cursor.close()
        conn.close()
...
def validate_ticket(ticket_id):
    ...
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute('UPDATE public."tickets" SET validatedOn = %s WHERE id = %s RETURNING *;', (validatedOn, ticket_id))
        updated_ticket = cursor.fetchone()
        conn.commit()
        cursor.close()
        conn.close()
